...
function PopularAttractionsCard({ attraction }) {
  ...
  return (
    <div
      className="relative min-w-[250px] max-w-[300px] text-black text-left border border-transparent rounded-xl bg-white dark:text-white dark:bg-gray-900 cursor-pointer"
      onClick={toggleModal}
    >
      <img
      ...
      <p className="font-light text-sm m-4 mt-0 truncate">{attraction.description}</p>
      <Modal
        isOpen={modalIsOpen}
        onRequestClose={toggleModal}
        contentLabel="Attraction Details"
        className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-11/12 md:max-w-3xl mx-auto bg-white p-8 rounded-md shadow-lg overflow-hidden z-50"
